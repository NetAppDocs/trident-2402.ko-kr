...
테스트 드라이브를 이용하려면 바로 사용할 수 있는 랩 이미지를 사용하여 "컨테이너화된 워크로드를 위한 영구 스토리지 간편 배포 및 복제" link:https://www.netapp.com/test-drives/["NetApp 시험 구동"^]에 대한 액세스 권한을 요청하십시오. 이 테스트 드라이브는 3개 노드로 구성된 Kubernetes 클러스터와 Astra Trident가 설치 및 구성된 샌드박스 환경을 제공합니다. Astra Trident에 익숙해지고 기능을 살펴보기에 좋은 방법입니다.
