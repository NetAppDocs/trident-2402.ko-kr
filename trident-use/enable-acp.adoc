...
Astra Trident가 이전에 설치되지 않은 클러스터를 Astra Control Service에 추가하면 해당 클러스터는 `Eligible`로 표시됩니다. 클러스터를 Astra Control에 추가하면 Astra Control Provisioner가 자동으로 활성화됩니다.
...
. [[no-registry]]원래 사용한 설치 방법을 사용하여 Astra Trident에서 Astra Control Provisioner를 활성화하십시오.
+
[role="tabbed-block"]
====
.Astra Trident 운영자
