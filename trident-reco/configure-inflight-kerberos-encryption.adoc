...
.시작하기 전에
* 관리형 Red Hat OpenShift 클러스터에서 Astra Control Provisioner가 활성화되어 있는지 확인하십시오. 지침은 link:../trident-use/enable-acp.html["Astra Control Provisioner를 활성화합니다"^]을 참조하십시오.
* 유틸리티에 액세스할 수 있는지 `tridentctl` 확인합니다.
